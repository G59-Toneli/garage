...
from the checkout (ADR-0004). Run Records in `eval/runs/` are generated by `eval run` and committed
with the change that moved them; CI re-measures and fails if the newest one no longer describes the
build. Promotion is deliberate and never automatic: `eval promote <run_id>`, in a human commit. The
fact format, the record format and the promotion procedure are documented in `docs/evaluation.md`.
